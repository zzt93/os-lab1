...
/**
   @param reqst_pid read from device dev_id == aim,
   @param offset cursor in file

   `reqst_pid`, starting at `offset`, read from device whose dev_id == aim,
   then copy into `buf`(which has `capacity` bytes)
...
/**
   @param offset -- meanless for tty_, for console itself know where to write
